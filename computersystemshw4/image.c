 ...
 * $Id: image.c 454 2011-11-28 23:02:54Z pjd $
 */
...
static int image_read(struct blkdev *dev, int offset, int len, char *buf)
{
    struct image_dev *im = dev->private;
    ...
    int result = pread(im->fd, buf, len*SECTOR_SIZE, offset*SECTOR_SIZE);
...
static int image_write(struct blkdev * dev, int offset, int len, char *buf)
{
    struct image_dev *im = dev->private;
    assert(im->magic == IMAGE_DEV_MAGIC);
    ...
    int result = pwrite(im->fd, buf, len*SECTOR_SIZE, offset*SECTOR_SIZE);
...
struct blkdev *image_create(char *path)
{
     ...
     */
    if (sb.st_size % SECTOR_SIZE != 0)
        fprintf(stderr, "warning: file %s not a multiple of %d bytes\n",
                path, SECTOR_SIZE);
    
    im->nblks = sb.st_size / SECTOR_SIZE;
